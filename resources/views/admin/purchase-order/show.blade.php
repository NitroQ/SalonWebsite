...
@section('body')
<h1 class="font-weight-bold"><a class="text-dark" href="{{route('admin.purchase-order.index')}}"><i class="fas fa-chevron-left mr-2"></i>Purchase Order Details {{$purchase_order->id}}</a></h1>
						...
						<div class="row mt-1">
							<div class="col-6 ml-auto"> 
								<div class="btn-group"> 
									...
									<a href="javascript:void(0);" onclick="confirmStatus('{{ route('admin.purchase-order.set-status', [$purchase_order->id, -1]) }}', -1);" class="btn btn-sm btn-outline-danger"><i class="fas fa-times-circle mr-2"></i>Reject</a>
									<a href="javascript:void(0);" onclick="confirmStatus('{{ route('admin.purchase-order.set-status', [$purchase_order->id, 1]) }}', 1);" class="btn btn-sm btn-outline-info"><i class="fas fa-check-circle mr-2"></i>Accept</a>
									{{-- @elseif($purchase_order->status == 1)
									<a href="javascript:void(0);" onclick="confirmStatus('{{ route('admin.purchase-order.set-status', [$purchase_order->id, 0]) }}', 0);" class="btn btn-sm btn-outline-warning"><i class="fas fa-search mr-2"></i>Processing</a>
